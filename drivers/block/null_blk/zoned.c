				...
				sector_t sector, unsigned int len)
{
	...
	pr_info("READ PROCESS in func %s: nr_sectors: %d,len: %d, SECTOR_SHIFT: %d",__func__,nr_sectors,len,SECTOR_SHIFT);
	pr_info("READ PROCESS : sector is: %d, WP is: %d", sector, zone->wp);
...
static blk_status_t null_zone_write(struct nullb_cmd *cmd, u64 z_offset,
				    unsigned int nr_bytes, bool append)
{
	struct nullb_device *dev = cmd->nq->dev;
	unsigned int zno = null_zone_no(dev, z_offset>>9);
	struct nullb_zone *zone = &dev->zones[zno];
	blk_status_t ret;
        pr_info("*FUNC %-sz_offset :%d,sector: %d, zno: %d",__func__,z_offset,z_offset>>9,zno);
	trace_nullb_zone_op(cmd, zno, zone->cond);
	u64 up_wp = nr_bytes;
	if (zone->type == BLK_ZONE_TYPE_CONVENTIONAL) {
		if (append)
			return BLK_STS_IOERR;
		return null_process_cmd(cmd, REQ_OP_WRITE, z_offset, nr_bytes);
	}
	...
	if (append) {
		...
		//sector = zone->wp;
		z_offset = zone->wp;
		pr_info("WRITE: FUNC %s	z_offset: %d, zone->wp: %d",__func__,z_offset,zone->wp);
		if (cmd->bio)
			cmd->bio->bi_iter.bi_sector = z_offset>>9;
		else
			cmd->rq->__sector = (z_offset>>9);
			//modify
			//cmd->rq->__sector = sector;
	} else if (z_offset != zone->wp) {
		ret = BLK_STS_IOERR;
		goto unlock;
	}

	if (zone->wp + nr_bytes > ((zone->start)*512 + (zone->capacity)*512)) {
		ret = BLK_STS_IOERR;
		goto unlock;
	}
	...
	ret = null_process_cmd(cmd, REQ_OP_WRITE, z_offset, nr_bytes);
	if (ret != BLK_STS_OK)
		goto unlock;
	//modify
	pr_info("WRITE: FUNC %s, up_wp: %d", __func__,up_wp);
	zone->wp += up_wp;
	//zone->wp += nr_sectors;
...
blk_status_t null_process_zoned_cmd(struct nullb_cmd *cmd, enum req_opf op,
				    u64 z_offset, u64 nr_bytes)
{	
	pr_info("{->WRITE: func %s, z_offset: %d, nr_bytes: %d",__func__, z_offset, nr_bytes);
	struct nullb_device *dev;
	struct nullb_zone *zone;
	blk_status_t sts;
	if(op == REQ_OP_WRITE)
		op = REQ_OP_ZONE_APPEND;
	pr_info("op is: %d,%s",op,op);
	switch (op) {
	case REQ_OP_WRITE:
		return null_zone_write(cmd, z_offset, nr_bytes, false);
	case REQ_OP_ZONE_APPEND:
		return null_zone_write(cmd, z_offset, nr_bytes, true);
	case REQ_OP_ZONE_RESET:
	case REQ_OP_ZONE_RESET_ALL:
	case REQ_OP_ZONE_OPEN:
	case REQ_OP_ZONE_CLOSE:
	case REQ_OP_ZONE_FINISH:
		return null_zone_mgmt(cmd, op, z_offset>>3);
	default:
		dev = cmd->nq->dev;
		zone = &dev->zones[null_zone_no(dev,z_offset>>3)];
