...
struct nullb_device {
	...
	u64	seq_wp;	
	// rock list
	struct rocks *rock_list;
	//static LIST_HEAD(rock_list);
	//struct rocks *rock_list;
};

/* modify define the linked-list struct*/
struct rocks{
                struct list_head rocks_list;
                unsigned long offset;	//offset for begin address of crresponding data
                unsigned long length;	//length for data size
};
